...
async function issueZPass(privateKey, inputs) {
  ...
  const issuerAcc = new Account({ privateKey });
  const issuerAddr = issuerAcc.address().to_string();
  ...
  const txId = await zpass.issueZPass({
    programName: "zpass_merkle_kaxxa123.aleo",
    functionName: "issue",
    inputs: [sig, leavesHashesString, issuerAddr],
    fee: 0.1,
    privateFee: false
  })
